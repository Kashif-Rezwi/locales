...
const TRANSLATABLE_ATTRS = new Set([
    ...
    'aria-placeholder', 'aria-description', 'label', 'content',
]);
...
const generate = (babelGenerator as unknown as { default: typeof babelGenerator }).default ?? babelGenerator;
...
export class CodeModService {
    ...
    transformCode(code: string, filePath: string, textToHash: Map<string, string>): string | null {
        ...
        try {
            ...
            this.logger.warn(`CodeMod: Babel parse failed for ${filePath} — skipping`);
            return null;
        }
        ...
        traverse(ast, {
            ...
            StringLiteral(path) {
                ...
                if (parent.type === 'JSXAttribute') {
                    const attrName = (parent as t.JSXAttribute).name;
                    const name = t.isJSXIdentifier(attrName) ? attrName.name : null;
                    if (name && (TRANSLATABLE_ATTRS.has(name) || (text.includes(' ') && text.length > 5))) {
                ...
                const dataParents = new Set(['VariableDeclarator', 'Property', 'ObjectProperty', 'ArrayExpression']);
                if (dataParents.has(parent.type) && text.includes(' ') && text.length > 5) {
                    path.replaceWith(buildTCall(hash));
                    substitutionCount++;
                }
            },
        ...
        (node) =>
            ...
            node.specifiers.some(
                (s) => t.isImportSpecifier(s) &&
                    (t.isIdentifier(s.imported) ? s.imported.name : s.imported.value) === 't',
            ),
    );
