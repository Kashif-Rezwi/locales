 ...
 */
@Injectable()
export class E2BWorkspaceProvider implements WorkspaceProvider, OnModuleDestroy {
    readonly name = 'e2b';
    ...
    async create(): Promise<WorkspaceHandle> {
        ...
        const sandbox = await (Sandbox as { create: (opts: Record<string, unknown>) => Promise<{ sandboxId: string } & Record<string, unknown>> }).create({ apiKey: this.apiKey });
    ...
    async clone(handle: WorkspaceHandle, repoUrl: string, branch: string): Promise<void> {
        const sandbox = this.activeSandboxes.get(handle.id) as {
            runCode: (cmd: string, opts?: unknown) => Promise<{ stdout: string; stderr: string; exitCode: number }>;
        } | undefined;

        if (!sandbox) throw new Error(`E2B: no active sandbox for workspace ${handle.id}`);

        const cmd = `git clone --depth 1 --branch ${branch} ${repoUrl} ${handle.workDir} 2>&1`;
        const result = await sandbox.runCode(cmd);

        if (result.exitCode !== 0) {
            throw new Error(`E2B clone failed (exit ${result.exitCode}): ${result.stdout}`);
        }
    ...
    async readFile(handle: WorkspaceHandle, filePath: string): Promise<string | null> {
        const sandbox = this.activeSandboxes.get(handle.id) as {
        ...
        try {
            ...
            const fs = (sandbox as Record<string, unknown>).files as { read: (p: string) => Promise<string> } | undefined
                ?? (sandbox as Record<string, unknown>).filesystem as { read: (p: string) => Promise<string> } | undefined;

            if (fs?.read) return await fs.read(absPath);

            // Fallback: cat via runCode
            const { runCode } = sandbox as { runCode: (cmd: string) => Promise<{ stdout: string; exitCode: number }> };
    ...
    async destroy(handle: WorkspaceHandle): Promise<void> {
        const sandbox = this.activeSandboxes.get(handle.id) as { kill?: () => Promise<void> } | undefined;
        if (!sandbox) return;
        try {
            await sandbox.kill?.();
            ...
            this.logger.warn(`Failed to destroy sandbox ${handle.id}: ${String(err)}`);
        }
    }
    ...
    async onModuleDestroy(): Promise<void> {
        ...
        this.logger.warn(`Cleaning up ${ids.length} surviving sandbox(es) on shutdown`);
        await Promise.allSettled(ids.map((id) => this.destroy({ id, workDir: '' })));
    }
}
