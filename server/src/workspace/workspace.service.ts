...
export class WorkspaceService implements OnModuleInit, OnModuleDestroy {
    ...
    async clone(handle: WorkspaceHandle, repoUrl: string, branch: string): Promise<void> {
        return this.withTimeout(() => this.provider.clone(handle, repoUrl, branch), 'clone');
    }
    ...
    async listFiles(handle: WorkspaceHandle, pattern: string): Promise<string[]> {
        return this.withTimeout(() => this.provider.listFiles(handle, pattern), 'listFiles');
    }
    ...
    async readFile(handle: WorkspaceHandle, filePath: string): Promise<string | null> {
        return this.withTimeout(() => this.provider.readFile(handle, filePath), 'readFile');
    }
    ...
    async destroy(handle: WorkspaceHandle): Promise<void> {
        try {
            ...
            this.logger.warn(`Workspace destroy failed for ${handle.id}: ${String(err)}`);
        }
    }
    ...
    private async withTimeout<T>(fn: () => Promise<T>, operation: string): Promise<T> {
        return Promise.race([
            ...
            new Promise<never>((_, reject) =>
                setTimeout(
                    () => reject(new Error(`Workspace ${operation} timed out after ${WORKSPACE_TIMEOUT_MS}ms`)),
