 ...
 */
@Injectable()
export class LocalWorkspaceProvider implements WorkspaceProvider, OnModuleDestroy {
    readonly name = 'local';
    ...
    async clone(handle: WorkspaceHandle, repoUrl: string, branch: string): Promise<void> {
        ...
        await git.clone(repoUrl, handle.workDir, ['--depth', '1', '--branch', branch]);
    ...
    async destroy(handle: WorkspaceHandle): Promise<void> {
        try {
            ...
            this.logger.warn(`Failed to remove workspace ${handle.workDir}: ${String(err)}`);
        }
    }
    ...
    async onModuleDestroy(): Promise<void> {
        ...
        this.logger.warn(`Cleaning up ${dirs.length} surviving local workspace(s) on shutdown`);
