...
const hashText = (text: string) => createHash('sha256').update(normalise(text)).digest('hex');
 ...
 */
function scoreConfidence(source: string, translation: string, providerConfidence: number): number {
...
export class TranslationEngineService {
    ...
    async translateBatch(
        ...
        if (inputs.length === 0) {
            return {
                results: [],
                metrics: { totalStrings: 0, tmHits: 0, tmMisses: 0, providerCalls: 0, providers: [], estimatedCostUsd: 0 },
            };
        }
        ...
        if (hits.length > 0) {
            ...
            this.prisma.translationMemory
                .updateMany({ where: { id: { in: hitIds } }, data: { usageCount: { increment: 1 } } })
                .catch((err: unknown) => this.logger.warn(`TM usageCount update failed: ${String(err)}`));
        }
        ...
        if (misses.length > 0) {
            ...
            for (let i = 0; i < misses.length; i++) {
                ...
                const confidence = scoreConfidence(miss.text, resp.translatedText, resp.confidence);
            ...
            // Run all TM upserts in parallel (fire-and-forget for performance)
            Promise.all(upserts).catch((err: unknown) =>
                this.logger.warn(`TM upsert batch failed: ${String(err)}`),
            );
        }
    ...
    async translateOne(
        ...
        const { results } = await this.translateBatch([{ text, hash }], sourceLocale, targetLocale, userId);
        return results[0];
    }
}
