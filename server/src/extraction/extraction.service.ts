...
function isTranslatable(raw: string, isJSX = false): boolean {
    ...
    if (words.length > 1 && words.every((w) => /^[a-z][a-z0-9_:-]*$/.test(w))) return false;
...
function regexFallback(code: string, filePath: string): SourceString[] {
    ...
    for (const [pattern, nodeType] of patterns) {
        ...
        while ((match = pattern.exec(code)) !== null) {
            ...
            seen.add(hash);
            results.push({ hash, sourceText: text, filePath, nodeType, context: 'regex-fallback' });
        }
    }
...
export class ExtractionService {
    ...
    async extractFromFiles(
        ...
        for (const filePath of jsxFiles) {
            ...
            const fileStrings = await this.extractFromCode(code, filePath);
        ...
        this.logger.debug(`Extracted ${results.length} unique strings from ${jsxFiles.length} files`);
        return results;
    }
     ...
     */
    private async extractFromCode(code: string, filePath: string): Promise<SourceString[]> {
        try {
            return this.extractWithBabel(code, filePath);
        } catch (err: unknown) {
            const message = err instanceof Error ? err.message : String(err);
            this.logger.warn(`Babel failed on ${filePath} (${message}) — using regex fallback`);
        ...
        traverse(ast, {
            ...
            StringLiteral(path) {
                ...
                if (parent.type === 'JSXAttribute') {
                    ...
                    if (typeof attrName === 'string' && TRANSLATABLE_ATTRS.has(attrName)) {
                        if (!isTranslatable(text, true)) return;
                        results.push({
                            hash: hashString(text),
                            sourceText: text,
                    ...
                    if (typeof attrName === 'string' && !TRANSLATABLE_ATTRS.has(attrName) && text.includes(' ') && text.length > 5) {
                ...
                const dataParentTypes = new Set(['VariableDeclarator', 'Property', 'ObjectProperty', 'ArrayExpression']);
                if (dataParentTypes.has(parent.type) && text.includes(' ') && text.length > 5) {
            ...
            TemplateLiteral(path) {
                ...
                const text = path.node.quasis[0]?.value.cooked?.replace(/\s+/g, ' ').trim();
                if (!text) return;
