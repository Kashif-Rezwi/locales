...
export class OpenAIProvider implements TranslationProvider {
    ...
    private async translateChunk(requests: TranslateRequest[]): Promise<TranslateResponse[]> {
        ...
        try {
            ...
            throw new ProviderError(`OpenAI network error: ${String(err)}`, ProviderErrorType.Retryable);
        }

        if (res.status === 429 || res.status >= 500) {
            throw new ProviderError(`OpenAI ${res.status}`, ProviderErrorType.Retryable, res.status);
        }
        if (res.status === 401 || res.status === 403) {
            throw new ProviderError(`OpenAI auth failure ${res.status}`, ProviderErrorType.Fatal, res.status);
        }
        if (!res.ok) {
            throw new ProviderError(`OpenAI error ${res.status}`, ProviderErrorType.Fatal, res.status);
        }
        ...
        try {
            ...
            throw new ProviderError('OpenAI returned invalid JSON', ProviderErrorType.Fatal);
        }
    ...
    estimateCost(wordCount: number, _targetLocale: string): CostEstimate {
        ...
        return { provider: this.name, estimatedCostUsd: wordCount * 1.3 * 0.00000015 };
    }
}
