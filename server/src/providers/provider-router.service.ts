...
const FAILED_RESULT = (_text: string): TranslateResponse => ({
  translatedText: '',
  provider: 'none',
  confidence: 0,
});
...
export class ProviderRouterService {
  ...
  /** Returns estimated cost in USD for translating `wordCount` words via the named provider. */
  estimateCost(
    providerName: string,
    wordCount: number,
    targetLocale: string,
    ...
    return provider.estimateCost(wordCount, targetLocale).estimatedCostUsd;
  }
  ...
  async translate(request: TranslateRequest): Promise<TranslateResponse> {
    ...
    this.logger.warn(
      `All providers failed for: "${request.text.slice(0, 40)}"`,
    );
  ...
  private async translateChunk(
    ...
    for (const provider of this.providers) {
      try {
        ...
        if (hasEmpty) {
          this.logger.warn(
            `${provider.name} returned empty translations — trying next provider`,
          );
          continue;
        }
        ...
        if (
          ...
          err.type === ProviderErrorType.Fatal
        ) {
          this.logger.warn(
            `${provider.name} fatal error (batch) — skipping: ${err.message}`,
          );
          continue;
        }
        this.logger.warn(
          `${provider.name} batch error — skipping: ${String(err)}`,
        );
        continue;
      }
    }
      ...
      try {
        ...
        if (err instanceof ProviderError) {
          if (err.type === ProviderErrorType.Fatal) {
            this.logger.warn(
              `${provider.name} fatal error — skipping: ${err.message}`,
            );
            return null;
          }
          ...
          this.logger.debug(
            `${provider.name} retryable error (attempt ${attempt + 1}) — retrying in ${delay}ms`,
          );
          await sleep(delay);
        } else {
          ...
          const delay = BACKOFF_MS[attempt] ?? 2000;
          this.logger.warn(
            `${provider.name} unknown error — retrying in ${delay}ms: ${String(err)}`,
          );
          await sleep(delay);
        }
      }
